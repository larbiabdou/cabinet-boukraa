...
class MedicalReportTemplate(models.Model):
    ...
    def get_default_content(self):
        ...
        templates = {
            'leave': '''
<p>Je soussigné, <strong>Dr {{ user.name }}</strong>, certifie avoir vu et examiné ce jour le (la) patient(e) <strong>{{ patient.name }}</strong></p>
<br/>
...
<br/>
<p>
    Permettez-moi de vous présenter le cas de mon patient, <strong>{{ patient.name }}</strong>, âgé de <strong>{{ age }}</strong> ans, que je vous confie pour {{ referral_reason }}.
</p>
<br/>
<p>
    <strong>Il s'est présenté à notre consultation pour :</strong><br/>
    {{ consultation_reason }}
</p>
<br/>
<p>
...
<p class="text-end">
    ...
    <strong>Signature :</strong> &#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;
        ...
        <p>Je soussigné, <strong>Dr {{ user.name }}</strong>, certifie avoir vu et examiné ce jour le (la) patient(e) <strong>{{ patient.name }}</strong>.</p>
        <br/>
        <p>Il s'est présenté à notre consultation pour <strong>{{ consultation_reason }}</strong>.</p>
        <br/>
